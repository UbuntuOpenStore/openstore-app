...
ClickPackageItem::~ClickPackageItem()
{
  // qDebug() << Q_FUNC_INFO << m_appId;
}

bool ClickPackageItem::install() const
{
  ...
  connect(installer, &ClickInstaller::busyChanged, this, [this, installer](){
    ...
    disconnect(installer, &ClickInstaller::downloadProgressChanged, nullptr, nullptr);
...
bool ClickPackageItem::remove() const
{
  qDebug() << Q_FUNC_INFO;
...
QString ClickPackageItem::appLaunchUrl() const
{
  if (m_installedVersion.isEmpty())
    return QString();

  Q_FOREACH (const HookStruct& h, m_hooks) {
    if (h.hooks & ClickPackageItem::HookDesktop) {
...
void ClickPackageItem::fillData(const QVariantMap& json)
{
  ...
  m_packageType = QStringLiteral("click");
